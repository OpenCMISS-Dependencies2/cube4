...
# AT_WITH_QT([QT_modules], [QT_config], [QT_misc], [RUN-IF-FAILED], [RUN-IF-OK], [ADDITIONAL_QT_MODULES])
# ------------------------------------------------------------------------------
...
dnl Memo: AC_ARG_WITH(package, help-string, [if-given], [if-not-given])
  AC_ARG_WITH([qt],
              [AS_HELP_STRING([--with-qt],
                 [Path to Qt @<:@Look in PATH, /usr/local/Trolltech, /usr/lib[|32|64]/qt[|4|5], /usr/local/qt[|4|5] and /opt/qt[|4|5]@:>@])],
...
dnl Memo: AC_ARG_WITH(package, help-string, [if-given], [if-not-given])
  ...
  test -d /usr/lib32/qt5 \
    && tmp_qt_paths="$tmp_qt_paths:/usr/lib32/qt5/bin"
  ...
  test -d /opt/qt5 \
    && tmp_qt_paths="$tmp_qt_paths:/opt/qt5/bin"

  test -d /usr/local/qt4 \
    && tmp_qt_paths="$tmp_qt_paths:/usr/local/qt4/bin"

  test -d /usr/local/qt5 \
    && tmp_qt_paths="$tmp_qt_paths:/usr/local/qt5/bin"
  ...
  # If we don't know the path to Qt, guess it from the path to qmake.
     if test ! -e $QT_PATH; then 
        QT_PATH=`dirname $QT_PATH`
        if test "x$QT_PATH" = "x."; then
        AX_INSTEAD_IF([$4],
...
_ASEOF
  ...
  # try out every additional modules separately 
  list_of_additiona_working_modules=""
  list_of_additiona_modules="$5";
  
  for add_module in $list_of_additiona_modules; do 
    # first cleanup as under Win7 qmake hungs 
    rm -rf Makefile* release debug $pro_file
    ...
    echo "Test whether $add_module is working" >&AS_MESSAGE_LOG_FD
    ...
    dnl Tweak the value of QT in the .pro if have been the 1st arg.
    m4_ifval([$add_module], [_AT_TWEAK_PRO_FILE([QT], [$add_module])])

    dnl Tweak the value of CONFIG in the .pro if have been given a 5nd arg.
    ...
    spec_argument=
    if $QMAKE -spec $QT_SPECS $at_qmake_args; then
    list_of_additiona_working_modules=" $add_module $list_of_additiona_working_modules"
    fi
    
  done 
    
    
  #rerun qmake to generate .pro file and try without additional modules
  if $QMAKE -project; then :; else
      AX_INSTEAD_IF([$4], [Calling $QMAKE -project failed.])
      break
  fi
...
dnl Tweak the value of QT in the .pro if have been the 1st arg.
m4_ifval([$list_of_additiona_working_modules], [_AT_TWEAK_PRO_FILE([QT], [$list_of_additiona_working_modules])])
...
[
 ...
 AC_CACHE_CHECK([for the QT_CXX to use with Qt], [at_cv_env_QT_CXX],
 [at_cv_env_QT_CXX=`sed "/^CXX@<:@^A-Z=@:>@*=/!d;$qt_sed_filter" $at_mfile`])
 
 AC_SUBST([QT_CXX], [`basename $at_cv_env_QT_CXX`])
 AS_CASE([$QT_CXX],
                            ["g++"],       [qt_compiler="gcc"],
                            ["xlC"],       [qt_compiler="ibm"],
                            ["clang++"],   [qt_compiler="clang"],
                            ["icpc"],      [qt_compiler="intel"],
                            ["openCC"],    [qt_compiler="open64"],
                            [AC_MSG_ERROR([Qt compiler "${QT_CXX}" is not supported by --with-frontend-compiler-suite.])])
