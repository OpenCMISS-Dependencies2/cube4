...
BSPOPHybridNoWaitINSTest::BSPOPHybridNoWaitINSTest( cube::CubeProxy* cube ) : PerformanceTest( cube )
{
...
void
BSPOPHybridNoWaitINSTest::applyCnode( const cube::Cnode* cnode, const cube::CalculationFlavour cnf,
...
void
BSPOPHybridNoWaitINSTest::applyCnode( const cube::list_of_cnodes& lcnodes,
                                      const bool                  direct_calculation  )
{
    ( void )direct_calculation; // not used here
    if ( no_wait_ins == nullptr )
...
const std::string&
BSPOPHybridNoWaitINSTest::getCommentText() const
{
    if ( no_wait_ins == nullptr )
    {
        return no_comment;
...
bool
BSPOPHybridNoWaitINSTest::isActive() const
{
    return no_wait_ins != nullptr;
};

bool
BSPOPHybridNoWaitINSTest::isIssue() const
{
    return false;
};
...
void
BSPOPHybridNoWaitINSTest::adjustForTest( cube::CubeProxy* cube ) const
{
    ( void )cube;
}
