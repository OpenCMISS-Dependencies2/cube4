                                                    ...
                                                    const QStringList&       dimnames )
{
    ...
    for ( uint i = 0; i < dims.size(); i++ )
    {
        ...
        grid->addWidget( vslider, 0, i + 1, Qt::AlignHCenter );
...
{
    QString label[] = { tr( "x" ), tr( "y" ), tr( "z" ) }; //names for the three dimensions
    int     w       = rect().height() / 7;
                                    ...
                                    int max ) : value_( value ), max_( max )
{
    ...
    push->setMinimumWidth( push->fontMetrics().boundingRect( "_999_" ).width() );
    push->setMaximumWidth( push->fontMetrics().boundingRect( "__999__" ).width() );
    setValue( value );
...
public:
    ...
    {
        return QSize( this->fontMetrics().boundingRect( "all" ).width(), 0 );
    }
protected:
    virtual void
