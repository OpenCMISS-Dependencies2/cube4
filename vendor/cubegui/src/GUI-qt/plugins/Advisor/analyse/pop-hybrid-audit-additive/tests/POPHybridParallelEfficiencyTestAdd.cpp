...
POPHybridParallelEfficiencyTestAdd::POPHybridParallelEfficiencyTestAdd(  cube::CubeProxy*                   cube,
                                                                         POPHybridProcessEfficiencyTestAdd* _proc_eff,
                                                                         POPHybridThreadEfficiencyTestAdd*  _thread_eff ) :
...
void
POPHybridParallelEfficiencyTestAdd::calculate( const cube::list_of_cnodes& cnodes )
{
    if ( pop_avg_comp == nullptr )
    {
        return;
...
void
POPHybridParallelEfficiencyTestAdd::calculate()
{
    ...
    setValue(
        ...
        ( ( thread_eff->isActive() ) ? thread_eff_value : 1. ) - 1
        );
}

void
POPHybridParallelEfficiencyTestAdd::applyCnode( const cube::Cnode*             cnode,
...
void
POPHybridParallelEfficiencyTestAdd::applyCnode( const cube::list_of_cnodes& cnodes,
                                                const bool                  direct_calculation  )
{
...
const std::string&
POPHybridParallelEfficiencyTestAdd::getCommentText() const
{
    if ( proc_eff == nullptr || thread_eff == nullptr  )
    {
        return no_comment;
...
bool
POPHybridParallelEfficiencyTestAdd::isActive() const
{
...
bool
POPHybridParallelEfficiencyTestAdd::isIssue() const
{
    return false;
};
...
void
POPHybridParallelEfficiencyTestAdd::adjustForTest( cube::CubeProxy* cube ) const
{
    ( void )cube;
}
