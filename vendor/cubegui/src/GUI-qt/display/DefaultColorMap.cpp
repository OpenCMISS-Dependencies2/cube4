...
DefaultColorMap::showDialog()
{
    ...
    if ( !colorDialog )
    {
        colorDialog = new ColorDialog( Globals::getMainWindow(), cw );
        connect( colorDialog, SIGNAL( apply() ), cw, SLOT( onApply() ) );
        connect( colorDialog, SIGNAL( cancel() ), cw, SLOT( onCancel() ) );
    }
    colorDialog->updateDialog();
...
ColorWidget::setColors()
{
    ...
    whiteStartCopy   = whiteStart;
    lightenStartCopy = lightenStart;
    for ( unsigned i = 0; i < 5; i++ )
    {
        colorPosCopy[ i ] = colorPos[ i ];
    }
    coloringMethodCopy = coloringMethod;
}
// end of setColors()

void
ColorWidget::onCancel()
{
    // if the dialog ended with "cancel", undo all changes
    setLightenStart( lightenStartCopy );
    setWhiteStart( whiteStartCopy );
    for ( unsigned i = 0; i < 5; i++ )
    {
        setColorPos( i, colorPosCopy[ i ] );
    }
    setColoringMethod( coloringMethodCopy );
...
ColorWidget::onApply()
{
    whiteStart   = getWhiteStart();
    lightenStart = getLightenStart();
    for ( unsigned i = 0; i < 5; i++ )
    {
        colorPos[ i ] = getColorPos( i );
    }
...
// if inDialog is true, then we paint a dialog color scale with additional information,
                          ...
                          ColorWidget* _colorWidget ) : QDialog( parent )
{
    ...
    QStringList label;
    label << tr( "Linear" ) << tr( "Quadratic 1" ) << tr( "Quadratic 2" ) << tr( "Exponential 1" ) << tr( "Exponential 2" );
    colorMethods << LINEAR << QUADRATIC1 << QUADRATIC2 << EXPONENTIAL1 << EXPONENTIAL2;
    for ( int i = 0; i < label.size(); i++ )
    {
        colorButton.append( new QRadioButton( label.at( i ) ) );
        vbox->addWidget( colorButton[ i ] );
    }

    int selectedIndex = colorMethods.indexOf( colorWidget->getColoringMethod() );
    selectedIndex = selectedIndex >= 0 ? selectedIndex : 0;
    colorButton[ selectedIndex ]->setChecked( true );

    connect( colorButton[ 0 ], SIGNAL( toggled( bool ) ), this, SLOT( onLinear( bool ) ) );
    connect( colorButton[ 1 ], SIGNAL( toggled( bool ) ), this, SLOT( onQuadratic1( bool ) ) );
    connect( colorButton[ 2 ], SIGNAL( toggled( bool ) ), this, SLOT( onQuadratic2( bool ) ) );
    connect( colorButton[ 3 ], SIGNAL( toggled( bool ) ), this, SLOT( onExponential1( bool ) ) );
    connect( colorButton[ 4 ], SIGNAL( toggled( bool ) ), this, SLOT( onExponential2( bool ) ) );

    QHBoxLayout* lightenLayout     = new QHBoxLayout();
    QLabel*      lightenStartLabel = new QLabel( this );
    lightenStartLabel->setText( tr( "Lighten colors for values under\nthis percentage of the maximal value:" ) );
    lightenLayout->addWidget( lightenStartLabel );

    lightenStartSpin = new QDoubleSpinBox( this );
    ...
    whiteStartLabel->setText( tr( "Use white to color values under\nthis percentage in the value range:" ) );
    whiteLayout->addWidget( whiteStartLabel );

    whiteStartSpin = new QDoubleSpinBox( this );
...
/**
 * initialize color dialog elements from values of colorWidget
 */
void
ColorDialog::updateDialog()
{
