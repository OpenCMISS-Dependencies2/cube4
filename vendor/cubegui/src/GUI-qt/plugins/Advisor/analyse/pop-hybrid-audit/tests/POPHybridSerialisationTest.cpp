...
POPHybridSerialisationTest::POPHybridSerialisationTest( cube::CubeProxy* cube ) : PerformanceTest( cube )
{
    setName( tr( " * * * Serialisation Efficiency" ).toUtf8().data() );
...
void
POPHybridSerialisationTest::applyCnode( const cube::Cnode*, const cube::CalculationFlavour,
                                        const bool )
{
}

void
POPHybridSerialisationTest::applyCnode( const cube::list_of_cnodes& cnodes,
...
double
POPHybridSerialisationTest::analyze( const cube::list_of_cnodes& cnodes,
...
const std::string&
POPHybridSerialisationTest::getCommentText() const
{
    if ( max_omp_serial_comp_time == nullptr || max_total_time_ideal == nullptr )
    {
        return no_comment;
...
bool
POPHybridSerialisationTest::isActive() const
{
    return ( max_omp_serial_comp_time != nullptr ) && ( max_total_time_ideal != nullptr );
};

bool
POPHybridSerialisationTest::isIssue() const
{
    return false;
};

void
POPHybridSerialisationTest::adjustForTest( cube::CubeProxy* cube ) const
{
...
void
POPHybridSerialisationTest::add_max_total_time_ideal_hyb( cube::CubeProxy* ) const
{
...
void
POPHybridSerialisationTest::add_transfer_time_mpi( cube::CubeProxy* ) const
{
    add_mpi_time( cube );
