...
POPHybridNoWaitINSTestAdd::POPHybridNoWaitINSTestAdd( cube::CubeProxy* cube ) : PerformanceTest( cube )
{
...
void
POPHybridNoWaitINSTestAdd::applyCnode( const cube::Cnode* cnode, const cube::CalculationFlavour cnf,
...
void
POPHybridNoWaitINSTestAdd::applyCnode( const cube::list_of_cnodes& lcnodes,
                                       const bool                  direct_calculation  )
{
    ( void )direct_calculation; // not used here
    if ( no_wait_ins == nullptr )
...
const std::string&
POPHybridNoWaitINSTestAdd::getCommentText() const
{
    if ( no_wait_ins == nullptr )
    {
        return no_comment;
...
bool
POPHybridNoWaitINSTestAdd::isActive() const
{
    return no_wait_ins != nullptr;
};

bool
POPHybridNoWaitINSTestAdd::isIssue() const
{
    return false;
};
...
void
POPHybridNoWaitINSTestAdd::adjustForTest( cube::CubeProxy* cube ) const
{
    ( void )cube;
}
